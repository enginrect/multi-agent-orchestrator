...
morch --config configs/adapters-mixed.yaml run task fix-auth-bug \
  --target-repo ~/repos/my-project \
  --description "Fix token refresh logic in auth module"

# Cursor step pauses (manual). Implement the changes, write the artifact,
# then resume:
morch --config configs/adapters-mixed.yaml resume task fix-auth-bug

# Claude and Codex run automatically. If approved:
morch task archive fix-auth-bug
```
...
morch --config configs/adapters-stub.yaml run task fix-auth-bug \
  --target-repo ~/repos/my-project
...
```bash
# 1. Create task
morch task init fix-auth-bug \
  --target-repo ~/repos/my-project \
  --description "Fix token refresh logic in auth module"

# 2. Edit scope
$EDITOR workspace/active/fix-auth-bug/00-scope.md

# 3. Implement changes in target repo, then write handoff
$EDITOR workspace/active/fix-auth-bug/01-cursor-implementation.md
morch task advance fix-auth-bug

# 4. Hand off to Claude — show instructions
morch task next fix-auth-bug

# 5. Claude writes review (on their machine/account)
$EDITOR workspace/active/fix-auth-bug/02-claude-review-cycle-1.md
morch task advance fix-auth-bug

# 6. If changes requested, Cursor responds
morch status task fix-auth-bug  # Check if rework needed
$EDITOR workspace/active/fix-auth-bug/03-cursor-response-cycle-1.md
morch task advance fix-auth-bug

# 7. Codex final review
$EDITOR workspace/active/fix-auth-bug/04-codex-review-cycle-1.md
morch task advance fix-auth-bug
