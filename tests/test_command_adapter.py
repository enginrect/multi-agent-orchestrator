...
class TestTimeoutConfiguration:
    """Tests for configurable timeout resolution and error messaging."""

    def test_default_timeout_used_when_no_override(self, store, task_dir):
        adapter = CommandAdapter(store, {"command": "echo", "args": [], "timeout": 42})
        ctx = _make_context()
        assert adapter._resolve_timeout(ctx) == 42

    def test_context_override_takes_precedence(self, store, task_dir):
        adapter = CommandAdapter(store, {"command": "echo", "args": [], "timeout": 42})
        ctx = _make_context()
        ctx["timeout_override"] = 1800
        assert adapter._resolve_timeout(ctx) == 1800

    def test_context_override_none_uses_adapter_default(self, store, task_dir):
        adapter = CommandAdapter(store, {"command": "echo", "args": [], "timeout": 500})
        ctx = _make_context()
        ctx["timeout_override"] = None
        assert adapter._resolve_timeout(ctx) == 500

    def test_timeout_error_includes_agent_and_phase(self, store, task_dir):
        adapter = CommandAdapter(store, {"command": "sleep", "args": ["999"], "timeout": 7})
        ctx = _make_context()
        ctx["agent"] = "codex"
        ctx["phase"] = "final_review"

        with patch("orchestrator.adapters.command.subprocess.run") as mock_run:
            mock_run.side_effect = subprocess.TimeoutExpired(cmd="sleep", timeout=7)
            result = adapter.execute("test-task", "01-impl.md", "tpl", "Do it", ctx)

        assert result.status == ExecutionStatus.FAILED
        assert "[codex]" in result.message
        assert "7s" in result.message
        assert "final_review" in result.message

    def test_cli_timeout_override_passed_to_subprocess(self, store, task_dir):
        """When timeout_override is in context, subprocess.run uses it."""
        adapter = CommandAdapter(store, {"command": "echo", "args": [], "timeout": 10})
        artifact = "01-impl.md"
        (task_dir / artifact).write_text("# Impl\n")

        ctx = _make_context()
        ctx["timeout_override"] = 3600

        with patch("orchestrator.adapters.command.subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            adapter.execute("test-task", artifact, "tpl", "Do it", ctx)

        call_kwargs = mock_run.call_args[1]
        assert call_kwargs["timeout"] == 3600

    def test_cursor_default_timeout_is_1200(self, store):
        adapter = CursorCommandAdapter(store)
        assert adapter._timeout == 1200

    def test_codex_default_timeout_is_900(self, store):
        adapter = CodexCommandAdapter(store)
        assert adapter._timeout == 900

    def test_claude_default_timeout_is_600(self, store):
        adapter = ClaudeCommandAdapter(store)
        assert adapter._timeout == 600

    def test_config_timeout_overrides_adapter_default(self, store):
        adapter = CursorCommandAdapter(store, settings={"timeout": 3600})
        assert adapter._timeout == 3600

    def test_spinner_receives_timeout_limit(self, store, task_dir):
        """Spinner is constructed with the effective timeout for display."""
        adapter = CommandAdapter(store, {"command": "echo", "args": [], "timeout": 777})
        artifact = "01-impl.md"
        (task_dir / artifact).write_text("# Impl\n")

        with patch("orchestrator.adapters.command.subprocess.run") as mock_run, \
             patch("orchestrator.adapters.command._ProgressSpinner") as mock_cls:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            mock_spinner = MagicMock()
            mock_cls.return_value = mock_spinner

            adapter.execute("test-task", artifact, "tpl", "Do it", _make_context())

            _, kwargs = mock_cls.call_args
            assert kwargs["timeout_limit"] == 777
