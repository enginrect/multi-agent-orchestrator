...
def comment_relayed_review(
    agent: AgentRole,
    pr_number: int,
    cycle: int,
    review_body: str,
) -> str:
    """Build a PR comment that relays an agent's review via the orchestrator.

    Used when the agent itself cannot reach GitHub (e.g. Codex sandbox)
    but the orchestrator can post on its behalf.
    """
    role = "reviewer" if agent == AgentRole.CLAUDE else "final reviewer"
    sig = agent_sig(agent.value)
    return (
        f"🤖 {sig} | Role: {role} | PR: #{pr_number} | Cycle: {cycle}\n"
        f"*(relayed by orchestrator — agent could not post directly)*\n\n"
        f"{review_body}"
    )
