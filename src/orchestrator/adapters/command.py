...
from ..domain.errors import classify_resource_error
...
_log = get_logger("adapters.command")

DEFAULT_TIMEOUT = 600  # 10 minutes — sensible default for most steps
...
class _ProgressSpinner:
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            limit_str = f"/{self._timeout_limit}s" if self._timeout_limit else ""
            msg = (
                f"\r{char} [{self._agent}] {self._phase} "
                f"| task: {self._task_name} "
                f"| elapsed: {int(elapsed)}s{limit_str}"
            )
            try:
...
class CommandAdapter(AgentAdapter):
    ...
    def _resolve_timeout(self, context: dict[str, Any]) -> int:
        """Resolve the effective timeout for this execution.

        Priority (highest wins):
        1. CLI ``--timeout`` override passed via ``context["timeout_override"]``
        2. Per-adapter ``settings.timeout`` (``self._timeout``)
        """
        override = context.get("timeout_override")
    ...
    def execute(
        ...
        effective_timeout = self._resolve_timeout(context)
        ...
        spinner = _ProgressSpinner(
            task_name, agent, phase, timeout_limit=effective_timeout,
        )
        spinner.start()
        ...
        try:
            ...
            return ExecutionResult(
                status=ExecutionStatus.FAILED,
                message=(
                    f"[{agent}] Timed out after {effective_timeout}s "
                    f"during '{phase}'. "
                    f"Increase timeout via --timeout flag or "
                    f"adapters.{agent}.settings.timeout in config."
                ),
            )
        ...
        if proc.returncode != 0:
            agent_name = context.get("agent", self.name)
            resource_err = classify_resource_error(
                agent_name, proc.stderr or "", proc.returncode,
            )
            if resource_err:
                _log.warning(
                    "Agent resource limit: %s",
                    resource_err,
                    extra={"agent": agent_name, "phase": artifact},
                )
                logger.log(
                    "adapter_resource_limit",
                    artifact=artifact,
                    error_type=type(resource_err).__name__,
                    agent=agent_name,
                    message=str(resource_err),
                    retry_after=resource_err.retry_after,
                    exit_code=proc.returncode,
                )
                retry_hint = ""
                if resource_err.retry_after:
                    retry_hint = f" (retry after {resource_err.retry_after}s)"
                return ExecutionResult(
                    status=ExecutionStatus.FAILED,
                    message=f"Resource limit: {resource_err}{retry_hint}",
                )
