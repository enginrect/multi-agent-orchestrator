...
from .infrastructure.setup_service import (
    SetupConfig,
    detect_all_agents,
    run_setup,
)
...
def _build_github_orchestrator(
    ...
    if timeout is not None:
        orch.timeout_override = int(timeout)
    return orch, config
...
def cmd_setup(args: argparse.Namespace) -> None:
    """Interactive setup for agent command paths."""
...
def cmd_config_show(args: argparse.Namespace) -> None:
    ...
    setup_config = SetupConfig.load()
    if setup_config.agent_paths:
        print(f"\nAgent command paths (~/.morch/config.yaml):")
        for name, path in sorted(setup_config.agent_paths.items()):
            print(f"  {name}: {path}")

    print(f"\nAgent detection:")
    for result in detect_all_agents():
        if result.installed:
            icon = "+" if result.authenticated else "~"
            auth = f" ({result.auth_message})" if result.auth_message else ""
            ver = f" v{result.version}" if result.version else ""
            print(f"  [{icon}] {result.name}: {result.path}{ver}{auth}")
        else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ---- setup ----
    p_setup = sub.add_parser("setup", help="Interactive agent setup and auto-detection")
    ...
    p_run_gh.add_argument("--timeout", type=int, default=None, help="Override agent timeout in seconds (e.g. 1800)")
    ...
    p_resume_github.add_argument("--timeout", type=int, default=None, help="Override agent timeout in seconds (e.g. 1800)")
    ...
    p_ghrun_compat.add_argument("--timeout", type=int, default=None)
    ...
    p_ghresume_compat.add_argument("--timeout", type=int, default=None)
...
def main() -> None:
    ...
    _log.info("Command: %s", " ".join(sys.argv[1:]))

    try:
        args.func(args)
    except OrchestratorError as e:
        _log.error("Orchestrator error: %s", e)
        print(f"Error: {e}", file=sys.stderr)
        sys.exit(1)
    except KeyboardInterrupt:
